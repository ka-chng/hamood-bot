...
class panda(commands.Cog): 
    ...
    @slash_command(name = 'panda', description = "Sends random media of panda\'s taken from r/panda")
    ...
    async def panda(self, ctx):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://www.reddit.com/r/panda/new.json?sort=hot') as r:
                res = await r.json()
                panda_embed = discord.Embed(
                color=embed_color
            )    
        panda_embed.set_thumbnail(url=embed_thumbnail)
        fortnite = res['data']['children'][random.randint(1, 25)]['data']['url']
        if ctx.channel.is_nsfw():
            await ctx.respond(embed=panda_embed)
        else:
